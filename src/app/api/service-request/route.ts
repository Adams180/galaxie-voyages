...
const VARIANTS: Record<string, { subject: string; fields: [string, string][] }> = {
  ...
  deposit: {
    ...
    fields: [
      ...
      ["phone", "Téléphone"],
      ["email", "E-mail"],
      ["pickup", "Lieu de prise en charge (aller)"],
      ["destination", "Destination (aller)"],
      ["dropoffDate", "Date de dépôt (aller)"],
      ["pickupDate", "Date de récupération (retour)"],
      ["returnDestination", "Destination au retour"],
      ["passengers", "Nombre de passagers"],
      ["busType", "Type de bus"],
      ["message", "Message"],
    ],
